...
% Pending items are marked with \tbd[...] and print in red. Search for "\tbd"
% before submitting.
...
stride-4 level (Table~4, Table~5; paired bootstrap 95\% CI $[4.11,\,5.49]$ at
...
\reply We have not added new detector families, for a reason that is central to
the paper: adding architectures without matched schedules, matched input sizes,
matched epoch budgets and paired tests would reproduce exactly the problem the
paper documents. Our experiments show that a schedule change alone moves mAP@50 by
about 4 points, which is larger than the margin most such comparisons report, so a
table of detectors trained under their own recipes cannot separate architecture
from protocol. Section~5.7 therefore reports published numbers with their
protocols attached and explicitly declines to rank them, and Section~6.4 lists the
narrow architectural scope --- one detector family, one backbone scale --- as a
...
manuscript and superseded entries were removed. \tbd[Authors: check the suggested
Applied Soft Computing paper and add it to Section~2.1 or~2.2 if it bears on
these questions; do not cite it unverified.]
...
improvements larger than about 1.3 points. Split by object size, the two-seed mean
change on COCO-small is $-0.80$ points, 95\% CI $[-1.50,\,-0.07]$ (Table~8).
...
with 62\% in one, so neither class can carry a conclusion; and UAVDT's objects are
larger, with 30.7\% of instances below 32 pixels against 68.6\% on the VisDrone
validation set, so the two benchmarks do not probe the same size regime.
...
(Sections~2.1--2.4). On the specific suggestions: this manuscript studies
single-sensor optical detection of objects in aerial images, and its measurements
concern detection stride, training resolution and module placement. The RF-sensing
and image--radar fusion works address a different sensing problem and a different
task, drone classification and anomaly detection rather than object detection in
imagery, so we could not connect them to a claim in the paper without padding the
review. A recent UAV-oriented DETR belongs to the same task and would fit
Section~2.1. \tbd[Authors: verify the UAV-DETR reference and add a sentence to
Section~2.1 if it checks out; we have not cited it unverified.]
...
\reply The reference list was rebuilt for the revised manuscript and now contains
28 entries, each cited at a specific claim in the text. The entry in question does
not appear in it. \tbd[Authors: confirm every entry in references.bib against its
